...
def complete_transcript():
    return parser_fixture() + pmm_fixture() + vm_fixture() + heap_fixture() + TIMER_OUTPUT + SCHED_GOOD + POST_IRQ
    ...
    def test_every_timer_line_required(self):
        for line in TIMER_OUTPUT.splitlines(keepends=True):
            with self.subTest(line=line):
                ...
                self.assertTrue(validate_boot_output(base_prefix() + timer + SCHED_GOOD + POST_IRQ))
