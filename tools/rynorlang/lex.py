...
def lex(source: str, filename: str = "<input>", edition: str = "v1") -> LexResult:
    ...
    # The size bound is bytes, and every character is at least one byte,
    # so a character count past the bound already proves a byte-count
    # violation without encoding (which could raise on lone surrogates).
    # Non-ASCII input at or under the bound still reaches the scanner's
    # own LEX_INVALID_CHAR; the bound only ever rejects inputs that are
    # also too large in bytes. lex_bytes() owns byte-exact handling for
    # callers that already hold encoded bytes.
    if len(source) > MAX_SOURCE_BYTES:
        return _too_large(filename, source[:MAX_SOURCE_BYTES + 1].encode("ascii", "replace"))
