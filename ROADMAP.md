...
Stages 0–19d are implemented within their documented scopes. Stage 19e onward
...
| 11 | Shell/monitor — implemented | Ring-0 kernel monitor with real IRQ1 keyboard input, bounded 64-byte line buffer and 12-token tokenizer, strict argument validation, `help`/`version`/`echo`/`clear` + `upper`/`count`/`digest` via Stage 10 services; synthetic and 39-key real QEMU interactive sessions with per-key `scan`/`ascii`/`line` and per-command `exec`/`result` evidence; at Stage 11: 110 repository + 155 integration test methods (147 non-shell + 8 shell), QEMU 9-configuration matrix, reproducible (current inventory 582/253). See `docs/reports/stage11.md`. |
...
| 17a | Block storage — implemented | IDE PIO block driver (PIIX3 compatibility, LBA28, polling, no DMA/IRQs) with PCI provenance, geometry validation, and per-request bounds; `BlockDevice` API (`read/write/capacity/block_size`, no controller leakage); deterministic patterned test images with host-recomputed digests; writes test-device-only with readback + neighbor proofs; timeout/error gauntlets and mutation matrix. See `docs/design/block-storage.md` and `docs/reports/stage17a.md`. No filesystem. |
| 17b | Native filesystem read — implemented | Original versioned RYNORFS v1 format (magic, version, geometry, 64-byte entries, flat storage with hierarchical resolution); validated superblock/directory/extents with overflow-safe arithmetic; path rules, handle model with generations, cross-block reads, EOF semantics; deterministic image builder plus independent host decoder; real QEMU reads through `blk_read` with host-recomputed digests; corruption/mutation matrices. See `docs/design/filesystem.md` and `docs/reports/stage17b.md`. No writes. |
| 17c | Native filesystem write — implemented | Overwrite-in-extent `fs_write` (no create/extend/truncate/delete; explicit `FS_RANGE` beyond EOF); partial-write reporting with completed-prefix counts; single-sector atomic unit, torn multi-block possible and tested via armed fault injection; no metadata changes (handles stay valid); static-only, generation-checked handles; deterministic write evidence with hex payloads plus remount readback and snapshot-off host-file corroboration. See `docs/design/filesystem.md` and `docs/reports/stage17c.md`. No journaling, no atomicity, no durability claims. |
| 18a | Protected userspace foundation — implemented | Static CPL3 foundation: user selectors + TSS/RSP0, per-context address spaces (private low replica via `vm_clone_low`, shared high snapshot re-synced at entry/resume/destroy), `int $0x80` exit/yield gate, user fault kill path with vector/error/CR2 record, IRQ0 preemption with faithful GPR resume (26 ticks / 28 switches / 13-13 / 14-14, tick-driven stop flags), SMEP/SMAP probe-only, OOM/admission rollback, two static contexts. No loader, no syscalls beyond the gate, no files, no processes. See `docs/design/userspace.md` and `docs/reports/stage18a.md`. |
| 18b | Process and executable loader + syscall boundary — implemented | RYNX v1 fixed-VA envelopes (host ELF converter, no kernel ELF parsing), validated loader (checked arithmetic, per-class reject reasons), fixed code/data/stack reuse with BSS zeroing, entry pinned to code base, no growth/sbrk; `int $0x80` syscall extension (exit/write/yield, frozen numbers, validated copyin, EAX return); three real programs (exit 42, hello write, fib27 with guaranteed preemption) plus a hostile syscall probe, 13-file malformed matrix, two-slot isolation proof, OOM rollback; no blocking (all syscalls non-blocking by construction — wait queues stay deferred with rationale). See `docs/design/executable-format.md`, `docs/design/syscall-abi.md`, and `docs/reports/stage18b.md`. No read, no argv/env, no runtime library. |
| 18c | Native runtime library — implemented | RynorLang-facing native library in CPL3 (`user/lib/rt/`: exit/write/yield wrappers, transactional `%s/%u/%x/%c` formatting, bounded 2048-byte arena with live ledger, cooperative nap/flag sync, honest `RT_NOSYS` stubs; library-local frozen `rt_err 0..5`, no new syscalls, no kernel sleep, no host APIs). In-OS targets built with `runtime="rtlib"` bind RynorLang print to `rt_write` (default `runtime="rynor"` preserves 18b bytes; host targets unchanged). Exit: seven CPL3 conformance programs with host-pinned bytes, kernel PMM/heap/table balance per program plus alloc live-count returning to 0 against a pinned monotonic watermark, five-mutant gate (3 fail-fast, 1 intended-timeout, 1 verified-with-write-row-mismatch), no host-API wrappers. See `docs/design/native-runtime.md`. |
| 18d | Native shell and REPL — implemented | Interactive RynorLang shell in CPL3 (per-block evaluation with whole-buffer re-analysis for deterministic symbols; session + per-submission arenas with zero-leak walks; `Ctrl-C` abort preserves session), file-backed scripts, true streaming pipelines with scheduler backpressure, and `$?`-style status query. The ring-0 monitor stays frozen and gains no evaluation; placement guard asserts no `kernel/shell/repl*` ever exists. Exit (verified, see `docs/reports/stage18d.md`): filesystem script test (`/test/session.sh` → `[SH] script done status=0`), keyboard interactive sessions, 8192-byte wrap/streaming proof with forced scheduler overlap (`overlap=1`) and bidirectional backpressure (`full≥1`/`empty≥1`), sequential-pipeline mutant deadlock, Ctrl-C editing/child/pipeline aborts, resident bounded evaluator with transactional commits plus `len(expr)`. Scripts are host-baked read-only files (RYNORFS v1 has no create/enumerate/growth); blocking reads and file-backed stdio stay reserved. |
| 19a | Aggregate language values — implemented (host-side) | `record` (nominal static shape), `list<T,N>` (fixed-cap array + length), `map<K,V,N>`, static sizes/offsets known at analyze time, inline storage with value semantics (no aliasing, no cycles, no inheritance, no null), plus string/byte operations (`len`, `byte_at`), pinned `int` semantics, and bitops. Exit (verified, see `docs/reports/stage19a.md`): per-type good/bad fixtures (16/34), exact-cap accept/reject pairs (8192-byte boundary both sides), static exhaustion yields `SEM_LIMIT_EXCEEDED` and runtime fallible ops yield `err` values (never trap), 13-mutant gate. Minimal `status` error values bridge to 19b `match`/`Result`; fixed-width integers beyond `int` stay deferred. |
| 19b | Match, control flow, and status — implemented (host-side) | `match` with narrowing (status/result/int/bool/str scrutinees, ok/err/literal/wildcard/binding patterns, exhaustiveness enforced), `break`/`continue` (innermost loop), `status`/`Result<T,E>` with explicit threading (`Error{code,message,span?,trace}` as a user-declared record convention; no `?` operator; no hidden exceptions), first-error diagnostics preserved. Exit (verified, see `docs/reports/stage19b.md`): narrowing tests, control-flow lowering goldens, transparent multi-call error propagation, 8-mutant gate. |
| 19c | Modules and std/manifest policy — implemented (host-side) | `use "path";` (file = module, mandatory stem alias, qualified `alias::name` calls/types, `{alias}__{name}` mangling), strict hash-pinned `rlmod.json` manifests (sha256 + per-module edition), cycle/duplicate/pin/edition errors, `std/` utility modules (`math`/`str`/`test`, pure, conformance-tested), frozen `std/io` + `std/mem` signatures with 19e-owned bindings, and the v1-only additive edition policy. Exit (verified, see `docs/reports/stage19c.md`): 7 good / 18 bad module projects with exact codes, 7 native-vs-oracle differentials, 6-mutant gate, v1 regression gate green. Guest/host memory and file API bindings stay deferred to 19e per the frozen RFC. |
| 19d | Language conformance and determinism — implemented (host-side) | Canonical encoding pinned for the full language (type strings, value bytes, print bytes, key bytes + FNV-1a64, AST-JSON order, RIR text, symbol numbering, asm-text contract; no-RNG/clock/env/filesystem-enumeration pin), bounded 83-cell type/rule/cap pairing matrix with ±1 cap evidence, 3x byte-identical host evidence (ungated text over the 35-program corpus; gated exit+stdout+exe-bytes with recorded aggregate hash), guest boundary evidence (rleval/rlen green + 3x session transcripts), `--profile=strict` lock (shell-constructs excluded, manifests required multi-file), seed plan (document), kLOC counting rule + counter (reference: host pipeline 8581 lines). Exit (verified, see `docs/reports/stage19d.md`): 33-test conformance suite green, 8-mutant gate, v1 regression gate green. Full-language guest execution and the core-dialect subset stay 19e-owned per the frozen RFC. |
...
0-11 kernel (PMM/VM/heap/threads/scheduler/keyboard/BGA text)
  ...
  |                   |
  |                   +--> 18b loader + syscalls (non-blocking only; blocking waits deferred)
  |                             |
