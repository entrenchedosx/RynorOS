...
records. The reviewed inventory contains 602 repository and 350 integration test
methods (Stage 18d Slices A/B: input path + syscall substrate; Slice C: processes + loader; Slice D: files + pipes; Slice E: CPL3 shell + scripts; Slice F: resident evaluator). The build command checks exact per-module participation before discovery. Exact commands and evidence are in the
